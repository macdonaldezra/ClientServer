...
int main (int argc, char** argv) {
    ...
    startListenerSocket(port, listenerSockDescriptor, succeeded);
    if (succeeded && listenerSockDescriptor != -1) {
        while(1) {
...
void usage(const char* prog) {
    cout << setw(6) << " "<<"Usage:  " << prog << " <server_port>\n"
         << setw(6) << " " << setfill('*') << setw(18) << "*"
         << " <server_port> is optional. Port 21 is used, when <server_port> is not mentioned.\n";
}
